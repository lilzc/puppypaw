    ...
    return (
      <div style={{ ...PAGE, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="relative z-10 w-full max-w-sm text-center">
          ...
          <div className="rounded-3xl p-8 anim-fade-sc" style={{ ...gc, boxShadow: '0 4px 16px rgba(0,0,0,0.1)' }}>
            ...
            {addInsurance && (
              <div className="rounded-2xl px-4 py-3 mb-4 flex items-center gap-3" style={{ background: '#F0FDF4', border: '1px solid rgba(34,197,94,0.18)' }}>
                <span style={{ fontSize: 18, flexShrink: 0 }}>🛡</span>
                <div className="text-left">
                  <p style={{ color: '#16A34A', fontWeight: 700, fontSize: 13 }}>宠物意外险已激活</p>
            ...
            <div className="rounded-2xl p-4 mb-5 text-left space-y-2" style={{ background: '#F9F8F6', border: '1px solid #E8E5E0' }}>
              ...
              <div className="flex justify-between text-sm pt-2" style={{ borderTop: '1px solid #F0EFED' }}>
          ...
          <div className="space-y-3 mb-6">
            {PAY_METHODS.map(m => (
              <button key={m.id} onClick={() => { setPayMethod(m.id); setPayStep('processing'); }}
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,107,44,0.35)'; e.currentTarget.style.background = '#F5F4F2'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = '#F5F4F2'; e.currentTarget.style.background = '#FFFFFF'; e.currentTarget.style.transform = 'none'; }}>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-black flex-shrink-0"
                    style={{ background: m.bg ?? '#F5F4F2', color: m.bg ? 'white' : undefined, fontSize: m.bg ? 18 : 26 }}>
                    {m.letter}
                  </div>
                  <div>
                  ...
                  <ChevronRight className="ml-auto flex-shrink-0" style={{ width: 18, height: 18, color: '#9B9B9B' }} />
                </div>
              </button>
            ))}
          </div>
      ...
      <div style={PAGE}>
        <div className="max-w-2xl mx-auto px-4 pt-6 pb-10">
          ...
          <div className="rounded-3xl p-6 mb-4" style={{ ...gc }}>
            <div className="flex items-center gap-4 mb-5 pb-5" style={{ borderBottom: '1px solid #F0EFED' }}>
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {selected.verified && (
                    <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: '#F0FDF4', color: '#16A34A', border: '1px solid #BBF7D0' }}>✓ 认证</span>
                  )}
                </div>
            ...
            {/* Handoff info row */}
            {handoff && (
              <div className="flex items-center gap-3 mb-4 px-1 pb-4" style={{ borderBottom: '1px solid #F0EFED' }}>
            ...
            <div className="space-y-3">
              ...
              {addInsurance && (
                <div className="flex justify-between text-sm">
                  ...
                  <span style={{ color: '#16A34A', fontWeight: 600 }}>¥3</span>
                </div>
              )}
              <div className="flex justify-between items-center pt-3" style={{ borderTop: '1px solid #F0EFED' }}>
                ...
                {addInsurance && (
                  <div className="space-y-1.5 text-xs" style={{ color: '#78716C', animation: 'fadeInUp 0.3s ease both' }}>
                    {['✓ 意外医疗费用最高 ¥50,000', '✓ 宠物死亡赔付 ¥10,000', '✓ 7×24h 紧急救援服务', '✓ 理赔到账最快 2 小时'].map(item => (
                      <p key={item} style={{ color: item.startsWith('✓') ? '#16A34A' : undefined }}>{item}</p>
                    ))}
                  </div>
                )}
              </div>
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                <span className="font-black text-base" style={{ color: '#16A34A' }}>¥3/次</span>
                {/* Toggle */}
                <button onClick={() => setAddInsurance(!addInsurance)}
                  className="relative transition-all duration-300"
                  style={{ width: 44, height: 24, borderRadius: 12, background: addInsurance ? 'linear-gradient(135deg,#16A34A,#22C55E)' : '#E8E5E0', border: 'none', cursor: 'pointer', boxShadow: addInsurance ? '0 3px 12px rgba(34,197,94,0.4)' : 'none' }}>
    ...
    return (
      <div style={PAGE}>
        <div className="max-w-2xl mx-auto px-4 pt-6">
          ...
          {/* Profile hero */}
          <div className="rounded-3xl p-6 mb-4 anim-fade-sc" style={{ ...gc, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
            <div className="flex items-start gap-5">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  {selected.verified && (
                    <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: '#F0FDF4', color: '#16A34A', border: '1px solid #BBF7D0' }}>
                ...
                <p style={{ color: '#78716C', fontSize: 13, lineHeight: 1.65 }}>{selected.bio}</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 mt-5 pt-4" style={{ borderTop: '1px solid #F0EFED' }}>
              {selected.tags.map(t => (
          ...
          {/* Booking form */}
          <div className="rounded-3xl p-6" style={{ ...gc, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
      ...
      <div className="max-w-3xl mx-auto">
        <div className="px-5 pt-7 pb-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs px-3 py-1 rounded-full font-semibold" style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', color: '#16A34A' }}>
              {filtered.length} 人在线
            </span>
          </div>
          <p className="text-sm flex items-center gap-1.5" style={{ color: '#78716C' }}>
        ...
        <div className="px-5 mb-5 flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
          {FILTER_CHIPS.map(fc => (
            <button key={fc.id} onClick={() => setFilter(fc.id)}
              ...
              style={filter === fc.id ? { background: 'linear-gradient(135deg,#FF6B2C,#FF9245)', color: '#1C1917', border: 'none', boxShadow: '0 4px 14px rgba(255,107,44,0.35)' } : { background: '#F9F8F6', color: '#78716C', border: '1px solid #E8E5E0' }}>
              {fc.label}
            </button>
          ))}
        </div>
        <div className="px-5 space-y-3">
          {filtered.map((w, i) => (
            <button key={w.id} onClick={() => setSelected(w)}
              ...
              onMouseEnter={e => { const el = e.currentTarget; el.style.background = '#F5F4F2'; el.style.borderColor = 'rgba(255,107,44,0.28)'; el.style.transform = 'translateY(-5px)'; el.style.boxShadow = '0 8px 24px rgba(0,0,0,0.1)'; }}
              onMouseLeave={e => { const el = e.currentTarget; el.style.background = '#FFFFFF'; el.style.borderColor = '#F5F4F2'; el.style.transform = 'translateY(0)'; el.style.boxShadow = 'none'; }}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      {w.verified && <span className="text-xs px-1.5 py-0.5 rounded-full font-semibold" style={{ background: '#F0FDF4', color: '#16A34A', fontSize: 10 }}>✓</span>}
                    </div>
                    <span className="font-black text-base" style={{ background: 'linear-gradient(135deg,#FF9245,#FFD166)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>¥{w.price}<span style={{ fontSize: 11, WebkitTextFillColor: '#A8A29E', fontWeight: 400 }}>/次</span></span>
                  </div>
                ...
                <ChevronRight style={{ width: 18, height: 18, color: '#C0BDB9', flexShrink: 0 }} />
              </div>
            </button>
          ))}
          {filtered.length === 0 && (
            <div className="rounded-3xl py-16 text-center" style={{ background: '#F5F4F2', border: '1px dashed #E8E5E0' }}>
              <div className="text-5xl mb-4 opacity-40">🐾</div>
              <p className="text-sm" style={{ color: '#9B9B9B' }}>没有找到符合条件的遛狗师</p>
            </div>
          )}
        </div>
      </div>
